...
<li class="card-txt-item"><p class="card-txt">downloads ${f}</p></li>
  ...
  </li>`).join("");a.insertAdjacentHTML("afterbegin",c);const o=new m(".gallery .gallery-link",{captionsData:"alt",captionPosition:"bottom",captionDelay:250});o.refresh(),o.on("show.simplelightbox",()=>{})}const L="51663153-45016a947364047e6aa27bf79";function b(r){y.get("https://pixabay.com/api/",{params:{key:L,q:r,image_type:"photo",orientation:"horizontal",safesearch:!0}}).then(a=>{if(a.data.hits.length===0)throw new Error;return x(a.data.hits),a.data.hits}).catch(a=>{p.error({position:"topRight",message:`Sorry, there are no images matching your search query. Please try again!
`})}).finally(()=>h())}const l=document.querySelector(".form");let s=null;l.addEventListener("input",r=>(s=String(r.target.value),s));l.addEventListener("submit",r=>{v(),r.preventDefault(),l.reset(),s===""&&(r.target.disabled=!0),g(),b(s)});
